...
class BasicLayer(BasicUnit):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        # TODO: Here I am confused about inplace vs not inplace
        if act_func == 'relu':
    ...
    def unit_str(self):
        ...
        if self.groups == 1:
            if self.dilation > 1:
                ...
                return '%dx%dx_Conv' % (kernel_size[0], kernel_size[1])
        else:
            if self.dilation > 1:
...
class MBInvertedConvLayer(BasicUnit):
    ...
    def forward(self, x):
        if self.inverted_bottleneck:
            if self.ngpu > 1:
                x = nn.parallel.data_parallel(self.inverted_bottleneck, x, range(self.ngpu))
            else:
                x = self.inverted_bottleneck(x)
        if self.ngpu > 1:
             x = nn.parallel.data_parallel(self.depth_conv, x, range(self.ngpu))
             x = nn.parallel.data_parallel(self.point_linear, x, range(self.ngpu))
        else:    
            x = self.depth_conv(x)
    ...
    @property
    def unit_str(self):
        unit_str = '%dx%x_MBConv%d' % (self.kernel_size, self.kernel_size, self.expand_ratio)
        return unit_str
    ...
    def get_flops(self, x):
        if self.inverted_bottleneck:
            flop1 = count_conv_flop(self.inverted_bottleneck, x)
            x = self.inverted_bottleneck(x)
        else:
            flop1 = 0
...
class ConvTransposeLayer(BasicUnit):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding='same', activation='relu6'):
        ...
        if padding == 'none':
            ...
            raise ValueError('[!]Unrecognized padding: ', padding)
        ...
        if activation == 'relu6':
            ...
            raise ValueError('[!]Unrecognized activation: %s'%(activation))
        self.transpose_conv = nn.Sequential(OrderedDict([
            ('conv_t', nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, pad, bias=False)),
            ('bn', nn.BatchNorm2d(out_channels)),
            ('activation', act),
        ]))
    ...
    def forward(self, x):
        if self.ngpu > 1:
            x = nn.parallel.data_parallel(self.transpose_conv, x, range(self.ngpu))
        else:
    ...
    @property
    def unit_str(self):
        unit_str = '%dx%x_ConvT' % (self.kernel_size, self.kernel_size)
        return unit_str
