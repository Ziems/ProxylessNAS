...
class ProxylessNasNet(BasicUnit):
    def __init__(self, first_conv, blocks, feature_mix_layer, output, flatten=True, pool=True, ngpu=0):
        ...
        self.first_conv.ngpu = ngpu
        self.output.ngpu = ngpu
        for b in self.blocks:
            b.mobile_inverted_conv.ngpu = ngpu
            if b.shortcut is not None:
                b.shortcut.ngpu = ngpu
    ...
    def forward(self, x):
        ...
        # Flatten should be done in the output
        x = self.output(x)
        return x
    ...
    @staticmethod
    def build_from_config(config, use_pool=True, use_flatten=True):
        first_conv = set_layer_from_config(config['first_conv'])
        feature_mix_layer = set_layer_from_config(config['feature_mix_layer'])
        output = set_layer_from_config(config['output'])
        ...
        return ProxylessNasNet(first_conv, blocks, feature_mix_layer, output, flatten, pool)
    ...
    def get_flops(self, x):
        ...
        if self.feature_mix_layer:
            ...
            flop += delta_flop
        x = self.global_avg_pooling(x)
        # TODO: Flatten should be done in the output, not here.
        x = x.view(x.size(0), -1) #flatten

        delta_flop, x = self.output.get_flops(x)
        flop += delta_flop
        return flop, x
    ...
    def init_model(self, model_init, init_div_groups=True):
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                if model_init == 'he_fout': # im not sure what this constant represents
